...
上课没完全听懂，本节内容主要抄自《数学天书中的证明》．

设我们有的无向图为 $G = (V, E)$．考虑 $G$ 的关联矩阵 $B = (b_{i, e})$，其行的指标集为 $V$，列的指标集为 $E$，有 $b_{i, e} = [i \in e]$．
...
其中 $N$ 取遍 $C$ 去掉第 $i$ 行后的所有 $(n - 1) \times (n - 1)$ 子矩阵．显然 $N$ 对应图 $G$ 中 $n$ 个顶点和 $n - 1$ 条边构成的一个子图，故只需证明
...
当这 $n - 1$ 条边生成树时，我们考虑一个删点序列 $j_1, \dots, j_{n - 1}$，使得每次按顺序删除点 $j_i$ 时均有 $j_i$ 的度数为 $1$，记删除点 $j_i$ 时移除的那条边为 $e_i$．现在置换行使得 $j_i$ 在第 $i$ 行，$e_i$ 在第 $i$ 列，由构造可知对于 $a < b$，有 $j_a \not\in e_b$，所以新的矩阵 $N^\prime$ 是下三角的，且对角线元素为 $\pm 1$，即 $\det(N) = \pm\det(N^\prime) = \pm 1$．得证．
